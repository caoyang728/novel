...
| 层级类型 | z-index | 说明 |
|---------|---------|------|
| 普通弹窗 | `1000 + level * 100` | 由 `showModal` 的 `level` 参数控制 |
| 确认弹窗 | 自动比当前最高弹窗 +1 | `showConfirmModal` 自动计算 |
| Loading / Login | 100000 | 最高级之一 |
| Toast | 100001 | 始终在最顶层 |
...
| | `hideLoading()` | 隐藏加载动画 |
| **弹窗** | `showModal(options)` | 通用弹窗（见下方说明） |
| | `showConfirmModal(options)` | 确认弹窗（见下方说明） |
| | `openModalById(id)` | 通过 ID 打开遗留模态框 |
| | `closeModalById(id)` | 通过 ID 关闭遗留模态框 |
...
#### `showModal(options)` — 通用弹窗

动态创建 DOM，不依赖页面预置 HTML。返回 `{ close, overlay, dialog }`。

```js
const modal = showModal({
    title: '标题',
    body: '<div>HTML内容</div>',       // 或 HTMLElement
    footer: '<button>确定</button>',   // 可选，不传则不显示
    width: '50%',                      // 可选，默认 50%
    height: '80vh',                    // 可选，默认 80vh
    minHeight: '0',                    // 可选，覆盖 CSS min-height（如确认弹窗）
    level: 0,                          // 可选，z-index = 1000 + level*100
    backdrop: 'rgba(30,41,59,0.6)',    // 可选，覆盖背景色
    blur: true,                        // 可选，默认 true
    closeOnBackdrop: false,            // 可选，默认 false
    closeOnEsc: true,                  // 可选，默认 true
    onConfirm: function(close) {},     // 可选，接收 close 函数
...
#### `showConfirmModal(options)` — 确认弹窗

基于 `showModal` 的快捷方法，自动管理层级（比当前最高弹窗高一级）。返回 `{ close, overlay, dialog }`。

```js
showConfirmModal({
    title: '确认删除',
    message: '确定要删除吗？',
    danger: true,                      // 可选，确认按钮用红色
    confirmText: '确认删除',           // 可选，默认 '确认'
