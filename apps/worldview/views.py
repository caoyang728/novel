...
from .prompts import (
    LAYER_CONFIGS, LAYER_NAMES,
    WORLDVIEW_GENERATE_ALL_LAYERS_SYSTEM_PROMPT,
    WORLDVIEW_GENERATE_ALL_LAYERS_USER_PROMPT,
    WORLDVIEW_GENERATE_SINGLE_LAYER_SYSTEM_PROMPT,
    WORLDVIEW_GENERATE_SINGLE_LAYER_USER_PROMPT,
    WORLDVIEW_STREAM_SINGLE_LAYER_SYSTEM_PROMPT,
    WORLDVIEW_STREAM_SINGLE_LAYER_USER_PROMPT,
    WORLDVIEW_STREAM_ALL_LAYERS_SYSTEM_PROMPT,
    WORLDVIEW_STREAM_ALL_LAYERS_USER_PROMPT,
    ...
    WORLDVIEW_GENERATE_OVERVIEW_SYSTEM_PROMPT,
    WORLDVIEW_GENERATE_OVERVIEW_USER_PROMPT,
    WORLDVIEW_GENERATE_PROFILE_SYSTEM_PROMPT,
    WORLDVIEW_GENERATE_PROFILE_USER_PROMPT,
    WORLDVIEW_GENERATE_RULES_SYSTEM_PROMPT,
    WORLDVIEW_GENERATE_RULES_USER_PROMPT,
...
class ApiWorldviewStructureView(BaseWorldAPIView):
    """世界观结构API """
    ...
    def get(self, request, pk):
        ...
        return self.success_response({
            'structure': self.get_world_profile(worldview),
            'axioms': self.get_world_profile(worldview).get('axioms', [])
        })
    ...
    def put(self, request, pk):
        ...
        if 'structure' in data:
            ...
            if 'identity' in profile:
                if 'identity' not in setting:
                    setting['identity'] = {}
                setting['identity']['identity'] = profile['identity']
            if 'tone' in profile:
                if 'position' not in setting:
                    setting['position'] = {}
                setting['position']['tone'] = profile['tone']
            if 'overview' in profile:
                setting['overview'] = profile['overview']
            if 'core_conflict' in profile:
                setting['conflict'] = profile['core_conflict']
            worldview.setting = setting
            worldview.save()

        return self.success_response(message='保存成功')


class ApiWorldviewLayersView(BaseWorldAPIView):
    """分层内容API"""

    def get(self, request, pk, layer_key=None):
        worldview = self.get_worldview(request.user, pk=pk)
        if not worldview:
            return self.error_response('世界观不存在')

        if layer_key:
            layer_data = self.get_layer_data(worldview, layer_key)
            content = layer_data.get('content', '') or layer_data.get('overview', '') or ''
            return self.success_response({
                'content': content,
                'status': 'saved'
            })

        return self.success_response(self.get_all_layers(worldview))

    def put(self, request, pk, layer_key=None):
        worldview = self.get_worldview(request.user, pk=pk)
        if not worldview:
            return self.error_response('世界观不存在')
        ...
        if layer_key and 'content' in data:
            self.set_layer_data(worldview, layer_key, {'content': data['content']})
            worldview.save()

            return self.success_response({'content': data['content']}, message='保存成功')

        return self.error_response('无效的请求')


class ApiWorldviewLayerConfirmView(BaseWorldAPIView):
    """确认单个分层"""

    def post(self, request, pk, layer_key):
        worldview = self.get_worldview(request.user, pk=pk)
        if not worldview:
            return self.error_response('世界观不存在')

        # WorldView 模型不需要确认状态，直接保存
        worldview.save()
        return self.success_response(message='确认成功')


class ApiWorldviewLayersSaveAllView(BaseWorldAPIView):
    """保存所有分层"""
    ...
    def post(self, request, pk):
        ...
        if 'layers' in data:
            for key, content in data['layers'].items():
                self.set_layer_data(worldview, key, {'content': content})
            worldview.save()
            return self.success_response(message='保存所有层成功')
        return self.error_response('缺少 layers 参数')


class ApiWorldviewLayersGenerateAllView(BaseWorldAPIView):
    """生成所有分层"""
    ...
    def post(self, request, pk):
        ...
        try:
            layers_prompt = '\n'.join([v['prompt'] for v in LAYER_CONFIGS.values()])
            
            prompt = ChatPromptTemplate.from_messages([
                ("system", WORLDVIEW_GENERATE_ALL_LAYERS_SYSTEM_PROMPT),
                ("user", WORLDVIEW_GENERATE_ALL_LAYERS_USER_PROMPT)
            ])
            ...
            result = chain.invoke({
                "name": self.get_world_name(worldview),
                "genre": self.get_world_genre(worldview) or '未设置',
                "description": self.get_world_summary(worldview) or '未设置',
                "core_conflict": self.get_world_conflict(worldview),
                "layers_prompt": layers_prompt,
            })
            ...
            if isinstance(result, dict):
                ...
                for key in LAYER_CONFIGS.keys():
                    generated[key] = f'{LAYER_CONFIGS[key]["name"]}内容生成...'
                    
        except Exception as e:
            logger.error(f"LLM调用失败: {e}")
            for key in LAYER_CONFIGS.keys():
                generated[key] = f'{LAYER_CONFIGS[key]["name"]}内容生成失败，请重试'
        
        for key, content in generated.items():
            self.set_layer_data(worldview, key, {'content': content})
        worldview.save()
        
        return self.success_response({'generated': generated})


class ApiWorldviewLayerGenerateView(BaseWorldAPIView):
    """生成单个分层"""
    ...
    def post(self, request, pk, layer_key):
        ...
        layer_info = LAYER_CONFIGS.get(layer_key, {'name': layer_key, 'description': ''})
        generated_content = ''

        try:
            prompt = ChatPromptTemplate.from_messages([
                ("system", WORLDVIEW_GENERATE_SINGLE_LAYER_SYSTEM_PROMPT),
                ("user", WORLDVIEW_GENERATE_SINGLE_LAYER_USER_PROMPT)
            ])
            ...
            logger.error(f"LLM调用失败: {e}")
            generated_content = f'{layer_info["name"]}内容生成失败，请重试'
        
        self.set_layer_data(worldview, layer_key, {'content': generated_content})
        worldview.save()
        
        return self.success_response({'content': generated_content})


class ApiWorldviewLayerGenerateStreamView(BaseWorldAPIView):
    """分层流式生成API"""
    
    def post(self, request, pk, layer_key=None):
        worldview = self.get_worldview(request.user, pk=pk)
        if not worldview:
            return self.error_response('世界观不存在', status=404)

        def generate():
            yield f"data: {json.dumps({'type': 'start', 'content': '开始生成内容..'}, ensure_ascii=False)}\n\n"
            ...
            try:
                ...
                for k in ['foundation', 'power', 'society', 'culture', 'history']:
                    if k != layer_key and k in all_layers:
                        content = all_layers[k].get('content', '') or all_layers[k].get('summary', '') or ''
                        if content:
                            existing_layers.append(f"[{k}] {content}")
                existing_layers_str = '\n'.join(existing_layers) if existing_layers else '暂无'

                prompt = ChatPromptTemplate.from_messages([
                    ("system", WORLDVIEW_STREAM_SINGLE_LAYER_SYSTEM_PROMPT),
                    ("user", WORLDVIEW_STREAM_SINGLE_LAYER_USER_PROMPT)
                ])
                ...
                stream_response = chain.stream({
                    ...
                    "description": self.get_world_summary(worldview) or '未设置',
                    "core_conflict": self.get_world_conflict(worldview),
                    "existing_layers_str": existing_layers_str,
                })
                ...
                # 保存到数据库
                self.set_layer_data(worldview, layer_key, {'content': full_content})
                worldview.save()
...
class ApiWorldviewAllLayersGenerateStreamView(BaseWorldAPIView):
    """所有分层流式生成API"""
    ...
    def post(self, request, pk):
        ...
        def generate():
            yield f"data: {json.dumps({'type': 'start', 'content': '开始生成所有分层内容..'}, ensure_ascii=False)}\n\n"
            
            try:
                layers_prompt = '\n'.join([v['prompt'] for v in LAYER_CONFIGS.values()])
                
                prompt = ChatPromptTemplate.from_messages([
                    ("system", WORLDVIEW_STREAM_ALL_LAYERS_SYSTEM_PROMPT),
                    ("user", WORLDVIEW_STREAM_ALL_LAYERS_USER_PROMPT)
                ])
                ...
                try:
                    ...
                    if json_match:
                        ...
                        yield f"data: {json.dumps({'type': 'error', 'message': '生成内容格式解析失败'}, ensure_ascii=False)}\n\n"
                        
                except Exception as e:
                    logger.error(f"JSON解析失败: {e}")
                    yield f"data: {json.dumps({'type': 'error', 'message': 'JSON解析失败: ' + str(e)}, ensure_ascii=False)}\n\n"
...
class ApiWorldviewGenerateOverviewView(BaseWorldAPIView):
    """AI生成世界总览"""
    ...
    def post(self, request, pk):
        ...
        name = data.get('name', self.get_world_name(worldview))
        genre = data.get('genre', self.get_world_genre(worldview))
        description = data.get('description', self.get_world_summary(worldview))
        background = data.get('background', self.get_world_conflict(worldview))
        
        try:
            prompt = ChatPromptTemplate.from_messages([
                ("system", WORLDVIEW_GENERATE_OVERVIEW_SYSTEM_PROMPT),
                ("user", WORLDVIEW_GENERATE_OVERVIEW_USER_PROMPT)
            ])
            ...
            if isinstance(result, dict):
                new_description = result.get('description', description)
                new_background = result.get('background', background)
            else:
...
class ApiWorldviewGenerateProfileView(BaseWorldAPIView):
    """AI生成世界概要（结构化设定）"""
    ...
    def post(self, request, pk):
        ...
        try:
            prompt = ChatPromptTemplate.from_messages([
                ("system", WORLDVIEW_GENERATE_PROFILE_SYSTEM_PROMPT),
                ("user", WORLDVIEW_GENERATE_PROFILE_USER_PROMPT)
            ])
            ...
            result = chain.invoke({
                "world_name": self.get_world_name(worldview),
                "genre": self.get_world_genre(worldview),
                "description": self.get_world_summary(worldview),
                "identity": identity,
                "tone": tone,
                "summary": summary,
            ...
            if isinstance(result, dict):
                new_identity = result.get('identity', identity)
                new_tone = result.get('tone', tone)
                new_summary = result.get('summary', summary)
                new_core_conflict = result.get('coreConflict', result.get('core_conflict', core_conflict))
            else:
                new_identity = identity
                new_tone = tone
                new_summary = summary
                new_core_conflict = core_conflict
        ...
        return self.success_response({
            ...
            'tone': new_tone,
            'summary': new_summary,
            'core_conflict': new_core_conflict
        })


class ApiWorldviewGenerateRulesView(BaseWorldAPIView):
    """AI生成世界规则"""
    ...
    def post(self, request, pk):
        ...
        try:
            prompt = ChatPromptTemplate.from_messages([
                ("system", WORLDVIEW_GENERATE_RULES_SYSTEM_PROMPT),
                ("user", WORLDVIEW_GENERATE_RULES_USER_PROMPT)
            ])
            ...
            result = chain.invoke({
                "name": self.get_world_name(worldview),
                "genre": self.get_world_genre(worldview) or '',
                "description": self.get_world_summary(worldview) or '',
                "count": count
            })
        ...
        return self.success_response({'summary': summary, 'rules': rules})
