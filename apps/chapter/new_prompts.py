# ==============================================================================
# 章节生成 Prompts（重构版：逐章生成，纯文本输出）
# ==============================================================================
#
# 改造要点：
# 1. Phase 2 从批次生成改为逐章生成，每章独立 LLM 调用
# 2. 输出格式从 JSON 分隔符改为纯文本，消除解析复杂度
# 3. System/User 分离保留（System 可命中 LLM 厂商 KV Cache）
# 4. 上下文标签重命名，消除 batch 概念
# 5. 新增 knowledge_context 字段（章节级知识库检索）
#
# 不变的部分：
# - Phase 1 概述生成（CHAPTER_OUTLINE_*）保持不变
# - 校验/修复/评分 prompt（单章级别）保持不变
# - 批量校验/修复 prompt（用于事后全量质检）保持不变
# ==============================================================================


# ==============================================================================
# Phase 1：生成章节标题 + 概述（不变，按卷一次性批量生成）
# ==============================================================================
...
#
# 设计原则：
# - System Prompt 包含静态基石信息（世界观+角色+卷大纲），可命中厂商 KV Cache
# - User Prompt 包含动态上下文（当前章概述、前后章、角色状态、时间线等），每章实时组装
# - 输出纯文本，不做 JSON 包装，彻底消除解析复杂度
# - 每章独立 LLM 调用，独享模型注意力，消除批次质量衰减

CHAPTER_CONTENT_SYSTEM_PROMPT = '''你是一位专业的小说创作家，擅长根据卷大纲和章节概述创作精彩的小说正文。
...
CHAPTER_CONTENT_USER_PROMPT = '''请根据以下上下文，撰写第{chapter_number}章的完整小说正文：
...
【重要】
- 仅输出第{chapter_number}章正文，不要输出其他章节内容
- 直接输出小说正文纯文本，不要JSON包装、不要markdown格式
- 不要提及章节标题或章节号
- 正文直接从场景描写、对话或叙述开始
- 注意与上一章末尾的自然衔接'''
...
# ==============================================================================
#
# 逐章生成方案中，该 prompt 用于每隔 N 章（建议 5-10 章）触发一次，
# 根据已生成的实际内容微调后续尚未生成的章节概述，保证概述与实际内容对齐。
# 硬约束保持不变：不得修改章节号、不得删除核心情节、不得改变阶段性结局。
...
CHAPTER_OUTLINE_ADJUST_USER_PROMPT = '''请根据最近生成的章节内容，对后续尚未生成的章节概述进行微调：

卷标题：{volume_title}
卷摘要：{volume_summary}

已完成章节摘要（最近{completed_count}章）：
{completed_summaries}
