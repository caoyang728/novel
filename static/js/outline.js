...
async function loadLatestOutline() {
    try {
        const data = await api.get(`/api/projects/${projectId}/outline/latest/`);
...
async function sendMessage() {
    ...
    try {
        const response = await fetch('/api/chat/outline/', {
            ...
            body: `project_id=${projectId}&version_number=${currentVersionNumber || 0}&message=${encodeURIComponent(message)}&current_outline=${encodeURIComponent(document.getElementById('outline-content').value)}&messages=${encodeURIComponent(JSON.stringify(messages))}`
        });
...
async function saveVersion(isNewVersion) {
    ...
    try {
        const data = await api.postForm('/api/ai/save/outline/version/', `project_id=${projectId}&content=${encodeURIComponent(content)}&messages=${encodeURIComponent(JSON.stringify(messages))}` + (isNewVersion ? '&new_version=true' : ''));
...
async function finalizeOutline() {
    try {
        const data = await api.postForm('/api/outline/finalize/', `project_id=${projectId}${currentVersionId ? `&version_id=${currentVersionId}` : ''}`);
