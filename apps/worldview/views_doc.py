...
from .models import WorldviewDoc, WorldviewDocChatHistory, WorldviewDocVersion
from .views import BaseWorldAPIView
from .prompts_doc import (
    WV_DOC_SYSTEM_PROMPT,
    WV_DOC_BUILD_PROMPT,
    WV_DOC_JSON_REPAIR_PROMPT,
    DOC_WELCOME_PROMPT,
    DOC_FACTION_EXTRACT_PROMPT,
    GENRE_LABELS,
    get_genre_guide,
    get_genre_label,
)
...
def _get_or_create_doc(project):
    """获取项目的世界观文档，不存在则创建（继承项目的题材类型）"""
    doc, created = WorldviewDoc.objects.get_or_create(
        project=project,
...
def _llm_repair_json(repair_llm, raw_content, error_msg):
    ...
    try:
        prompt = ChatPromptTemplate.from_messages([
            ("human", WV_DOC_JSON_REPAIR_PROMPT),
        ])
...
class ApiWorldviewDocView(BaseWorldAPIView):
    """世界观文档 GET/PUT"""

    def get(self, request, project_id):
        project = self.get_project_or_404(request, project_id)
        doc = WorldviewDoc.objects.filter(project=project, is_deleted=False).first()
        if not doc:
...
class ApiWorldviewDocChatOpenView(BaseWorldAPIView):
    ...
    def post(self, request, project_id):
        ...
        prompt = ChatPromptTemplate.from_messages([
            ...
            ("human", DOC_WELCOME_PROMPT),
        ])

        try:
            llm = get_llm(user=request.user, scene="worldview_doc_chat")
            chain = prompt | llm
            current_doc_section = ''
            ...
            self.log_token_usage('worldview_doc_welcome', result=result,
...
class ApiWorldviewDocChatStreamView(BaseWorldAPIView):
    ...
    def post(self, request, project_id):
        ...
        def generate():
            try:
                close_old_connections()
                doc = _get_or_create_doc(project)
                current_version = WorldviewDocVersion.objects.filter(
                    doc=doc, is_current=True, is_deleted=False
                ).first()
                ...
                # 持久化用户消息
                WorldviewDocChatHistory.objects.create(
                    doc=doc, version=current_version, role='user', content=user_input
                )

                prompt = ChatPromptTemplate.from_messages([
                    ("system", WV_DOC_SYSTEM_PROMPT),
                    ("human", WV_DOC_BUILD_PROMPT),
                ])

                llm = get_llm(user=request.user, scene="worldview_doc_build")
                # 修复用 LLM：低温度，输出更稳定
                repair_llm = get_llm(user=request.user, scene="worldview_doc_chat", temperature=0.1)
                ...
                for round_idx in range(1, MAX_ROUNDS + 1):
                    ...
                    round_chain = chain
                    if round_idx > 1:
                        round_llm = get_llm(user=request.user, scene="worldview_doc_build", temperature=0.4)
                    ...
                    # token 统计
                    try:
                        self.log_token_usage('worldview_doc_build', result=last_chunk,
                ...
                # 持久化助手消息
                WorldviewDocChatHistory.objects.create(
                    doc=doc, version=current_version, role='assistant',
...
class ApiWorldviewDocFactionExtractView(BaseWorldAPIView):
    ...
    def post(self, request, project_id):
        ...
        prompt = ChatPromptTemplate.from_messages([
            ...
            ("human", DOC_FACTION_EXTRACT_PROMPT),
        ])
        ...
        try:
            ...
            self.log_token_usage('worldview_doc_faction_extract', result=result,
                                 user=request.user, project=project)
...
class ApiWorldviewDocVersionsView(BaseWorldAPIView):
    """世界观文档版本列表"""
    ...
    def get(self, request, project_id):
        ...
        doc = WorldviewDoc.objects.filter(project=project, is_deleted=False).first()
        if not doc:
            return self.success_response({'versions': [], 'latest': None})

        versions = WorldviewDocVersion.objects.filter(doc=doc, is_deleted=False)
        data = [{
            'id': v.id,
...
class ApiWorldviewDocVersionLoadView(BaseWorldAPIView):
    """加载指定版本"""
    ...
    def get(self, request, project_id, version_id):
        ...
        doc = WorldviewDoc.objects.filter(project=project, is_deleted=False).first()
        if not doc:
            return self.error_response('文档不存在')

        version = WorldviewDocVersion.objects.filter(
            id=version_id, doc=doc, is_deleted=False
        ).first()
        if not version:
...
class ApiWorldviewDocVersionSaveView(BaseWorldAPIView):
    """保存新版本"""
    ...
    def post(self, request, project_id):
        ...
        content = (request.data.get('content') or doc.content or '').strip()
        if not content:
            return self.error_response('内容不能为空')

        next_vn = (WorldviewDocVersion.objects.filter(doc=doc).aggregate(
            models.Max('version_number'))['version_number__max'] or 0) + 1

        # 从请求中获取 last_question/last_options（由保存时传入）
        last_question = (request.data.get('last_question') or '').strip()
        last_options = request.data.get('last_options') or []

        WorldviewDocVersion.objects.filter(doc=doc, is_current=True).update(is_current=False)
        version = WorldviewDocVersion.objects.create(
            doc=doc,
        ...
        doc.content = content
        doc.version = next_vn
        doc.save(update_fields=['content', 'version', 'updated_at'])
...
class ApiWorldviewDocVersionUpdateView(BaseWorldAPIView):
    """更新当前版本内容（不创建新版本）"""
    ...
    def post(self, request, project_id):
        ...
        if not version:
            return self.error_response('没有当前版本')
...
class ApiWorldviewDocVersionLockView(BaseWorldAPIView):
    """锁定版本"""
...
class ApiWorldviewDocVersionUnlockView(BaseWorldAPIView):
    """解锁版本"""
...
class ApiWorldviewDocVersionDeleteView(BaseWorldAPIView):
    """删除版本"""
    ...
    def post(self, request, project_id, version_id):
        ...
        if version.is_current:
            latest = WorldviewDocVersion.objects.filter(
            ...
            if latest:
                ...
                doc.version = latest.version_number
                doc.save(update_fields=['content', 'version', 'updated_at'])

        return self.success_response({'deleted': True})


class ApiWorldviewDocChatHistoryView(BaseWorldAPIView):
    """获取最近的聊天历史（最后一条 assistant 消息及选项）"""
    ...
    def get(self, request, project_id):
        ...
        try:
            ...
            limit = 10
        histories = WorldviewDocChatHistory.objects.filter(
            doc=doc, is_deleted=False
